...
// Get references to the search bar and all tags
...
tags.forEach(tag => {
  tag.addEventListener('click', () => {
    // Insert the tag value into the search bar
    const tagValue = tag.getAttribute('data-value');
    if (!searchBar.value.includes(tagValue)) {
      searchBar.value += (searchBar.value ? ', ' : '') + tagValue;
    }
  });
});
