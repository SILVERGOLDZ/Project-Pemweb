<?php
include 'conn.php';
$SUPABASE_URL = "https://kfkqpjzxvkbdnhrxlnbr.supabase.co"; 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($_FILES['profileImage']['type'], $allowedTypes)) {
        echo "<script>alert('Invalid file type. Only JPG, PNG, and GIF files are allowed.'); window.history.back();</script>";
        exit();
    }


    $filePath = $_FILES['profileImage']['tmp_name'];
    $fileName = urlencode(basename($_FILES['profileImage']['name']));
